<template>
  <footer>
    <div class="warp_up">
      <div class="bilibili item">
        <div class="title">bilibili</div>
        <ul>
          <li>关于我们</li>
          <li>友情链接</li>
          <li>哔哩哔哩周边</li>
          <li>联系我们</li>
          <li>加入我们</li>
          <li>官方认证</li>
        </ul>
      </div>
      <div class="link item">
        <div class="title">传送门</div>
        <ul>
          <li>帮助中心</li>
          <li>高级弹幕</li>
          <li>活动专题页</li>
          <li>侵权申诉</li>
          <li>分院帽计划</li>
          <li>活动中心</li>
          <li>用户反馈论坛</li>
          <li>壁纸站</li>
          <li>名人堂</li>
        </ul>
      </div>
      <div class="icon item">
        <ul>
          <li><div class="mobile"></div>手机端下载</li>
          <li><div class="weibo"></div>新浪微博</li>
          <li><div class="wechat"></div>官方微信</li>
        </ul>
      </div>
    </div>
    <div class="warp_down">
      <div class="pic"></div>
      <div class="picRight">
        <p>广播电视节目制作经营许可证：（沪）字第1248号 | 网络文化经营许可证：沪网文[2013]0480-056号 | 信息网络传播视听节目许可证：0910417 | 互联网ICP备案：沪ICP备13002172号-3</p>
        <p>沪ICP证：沪B2-20100043 | 违法不良信息举报邮箱：help@bilibili.com | 违法不良信息举报电话：4000233233转3</p>
        <p> 上海互联网举报中心 |12318 全国文化市场举报网站 |<img src="//static.hdslb.com/images/base/beiantubiao.png" style="vertical-align: top;"> 沪公网安备 31011502001911号 |儿童色情信息举报专区</p>
        <p>网上有害信息举报专区：<img src="//static.hdslb.com/images/12377.png" width="16" height="16" style="vertical-align: sub;"> 中国互联网违法和不良信息举报中心</p>
      </div>
    </div>
  </footer>
</template>

<script>
    export default {
      name: ''
    }
</script>

<style lang="scss" scoped>
  @import "../common/style/variable";

  footer {
    width: 100%;
    background-color: $footerGrey;
    padding-top: 20px;
    font-size: 13px;
    padding-bottom: 30px;
    div.warp_up{
      width: 1160px;
      margin: 0 auto;
      display: flex;
      div.item {
        flex: 1;
        border-right: 1px solid $fontGrey;
        div.title {
          padding: 5px 25px 10px;
          color: $fontDark;
        }
        li {
          width: 80px;
          padding: 10px 20px;
          float: left;
          cursor: pointer;
          &:hover {
            color: $fontActive;
          }
          div.mobile {
            width: 60px;
            height: 60px;
            background: url("//static.hdslb.com/images/base/icons.png") no-repeat -1024px -194px;
            &:hover {
              background-position: -1090px -194px;
            }
          }
          div.weibo {
            width: 60px;
            height: 60px;
            background: url("//static.hdslb.com/images/base/icons.png") no-repeat -1024px -322px;
            &:hover {
              background-position: -1090px -322px;
            }
          }
          div.wechat {
            width: 60px;
            height: 60px;
            background: url("//static.hdslb.com/images/base/icons.png") no-repeat -1024px -66px;
            &:hover {
              background-position: -1090px -66px;
            }
          }
        }
      }
      div.icon {
        ul {
          display: flex;
          justify-content: space-around;
          li {
            padding: 10px;
            div {
              padding-bottom: 10px;
            }
          }
        }
      }
    }
    div.warp_down {
      padding-top: 30px;
      font-size: 12px;
      display: flex;
      width: 1160px;
      margin: 0 auto;
      div.pic {
        background: url(data:image/png;base64,) no-repeat;
        width: 100px;
        height: 80px;
      }
      div.picRight {
        flex: auto;
        padding-left: 30px;
        color: $fontDark;
        p {
          padding: 3px 0;
        }
      }
    }
  }
</style>
